...
fn load_output_layer_safetensor<B: Backend>(
    dense_weight: &CandleTensor,
    dense_bias: &CandleTensor,
    layer_norm_bias: &CandleTensor,
    layer_norm_weight: &CandleTensor,
    device: &B::Device,
) -> BertOutputRecord<B> {
...
fn load_intermediate_layer<B: Backend>(
    layer_dir: &str,
    device: &B::Device,
) -> BertIntermediateRecord<B> {
...
fn load_self_attention_layer<B: Backend>(
    layer_dir: &str,
    device: &B::Device,
) -> BertSelfAttentionRecord<B> {
...
fn load_self_attention_layer_safetensor<B: Backend>(
    query_weight: &CandleTensor,
    query_bias: &CandleTensor,
    key_weight: &CandleTensor,
    key_bias: &CandleTensor,
    value_weight: &CandleTensor,
    value_bias: &CandleTensor,
    device: &B::Device,
) -> BertSelfAttentionRecord<B> {
...
fn load_self_output_layer<B: Backend>(
    layer_dir: &str,
    device: &B::Device,
) -> BertSelfOutputRecord<B> {
...
fn load_self_output_layer_safetensor<B: Backend>(
    dense_weight: &CandleTensor,
    dense_bias: &CandleTensor,
    layer_norm_bias: &CandleTensor,
    layer_norm_weight: &CandleTensor,
...
fn load_attention_layer_safetensor<B: Backend>(
    ...
    device: &B::Device,
) -> BertAttentionRecord<B> {
    let self_attention = load_self_attention_layer_safetensor::<B>(
    ...
    let self_output = load_self_output_layer_safetensor::<B>(
...
fn load_encoder<B: Backend>(encoder_dir: &str, device: &B::Device) -> BertEncoderRecord<B> {
    ...
    for i in 0..num_layers {
        ...
        let attention_layer =
            load_attention_layer::<B>(format!("{}attention/", layer_dir).as_str(), device);
        let intermediate_layer =
            load_intermediate_layer::<B>(format!("{}intermediate/", layer_dir).as_str(), device);
...
fn load_encoder_from_safetensors<B: Backend>(
    ...
    for (key, value) in layers.iter() {
        ...
        let intermediate_layer = load_intermediate_layer_safetensor::<B>(
            &value[&format!("{}.intermediate.dense.weight", layer_key)],
            &value[&format!("{}.intermediate.dense.bias", layer_key)],
            device,
        );
        let output_layer = load_output_layer_safetensor::<B>(
...
fn load_embeddings<B: Backend>(
    embeddings_dir: &str,
    device: &B::Device,
) -> BertEmbeddingsRecord<B> {
    let word_embeddings =
        load_embedding::<B>(&format!("{}word_embeddings/", embeddings_dir), device);
    let position_embeddings =
        load_embedding::<B>(&format!("{}position_embeddings/", embeddings_dir), device);
    let token_type_embeddings =
        load_embedding::<B>(&format!("{}token_type_embeddings/", embeddings_dir), device);
