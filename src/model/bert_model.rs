...
impl<B: Backend> BertModel<B> {
    ...
    pub fn forward(
        &self,
        input: BertEmbeddingsInferenceBatch<B>,
        device: &B::Device,
    ) -> Tensor<B, 3> {
        ...
        let mut mask_attn: Tensor<B, 2> =
            Tensor::ones(shape.clone(), device).to_device(&input.tokens.device());
    ...
    pub fn get_extended_attention_mask(
        ...
        let min_val = f32::MIN;
        extended_attention_mask = (Tensor::<B, 4>::ones(extended_attention_mask.shape(), &device)
            .to_device(&device.clone())
